...
fn emit_event(
    ...
    let mut sites: Vec<&str> = ev.call_sites.iter().map(|c| c.module.as_str()).collect();
    sites.dedup();
    if !sites.is_empty() {
...
mod tests {
    ...
    fn generated_wam_is_valid_rust() {
        ...
        assert!(code.contains(
            "WamGlobalDef { name: \"psId\", id: 6005, channels: &[WamChannel::Private] }"
        ));
