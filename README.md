...
**`schemaVersion` is now 4.3.0**, a minor: response variants gain an optional `child` assertion, skipped when the parser reads payload off every child it gates on. It records a pure presence gate - `var r = flattenedChildWithTag(node, "tag"); if (!r.success) return r;` where `r.value` is never read: the parser branches on the child's presence without decoding anything off it, so no field is emitted for it and without the assertion two variants collapse into one shape. The motivating case is `AcceptGroupAdd`, whose join-request success requires `<membership_approval_request>` while the bare `<iq type="result">` success requires nothing - previously identical in the IR, which is how an accepted join surfaced as a parse error downstream. Read structurally from the `oxc` AST like every other gate (guarded binding, no `.value` read anywhere in the body), never from the variant name; a gate whose value is read stays payload and records nothing. The reference Rust consumer enforces it: the generated success guard bails when the child is absent, so the RPC cascade falls through to the childless variant instead of accepting the wrong shape. Like the earlier `reference` widening, this widens the value space of an existing field: a closed-enum consumer rejects documents carrying it until it handles the new kind, which is better than silently merging two variants into one shape.
