...
- **What the extractor could not resolve says so.** `target` is `unknown` when the request writes a `to` that resolves to no fixed server (the newsletter requests address `WAWap.JID(newsletterId)`) and `unset` when nothing writes one at all, instead of both reading `s.whatsapp.net`; a field with no accessor whose content is its children is `type: "node"` instead of `string`. Both counted in the manifest, both guarded.
