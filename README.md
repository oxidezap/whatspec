...
- **What the extractor could not resolve says so.** `target` is `unset` when the builder writes no `to` and `unknown` when it writes one that resolves to no server, instead of both reading `s.whatsapp.net`; a field with no accessor whose content is its children is `type: "node"` instead of `string`. Both counted in the manifest, both guarded.
